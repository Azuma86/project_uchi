...
/**
 * 画面上部のヘッダー。
 * モバイルでは家族名とアカウント、PC ではページタイトルを主に表示する。
 */
    ...
    <header className="sticky top-0 z-20 border-b border-line bg-cream/90 backdrop-blur">
      <div className="mx-auto flex min-h-[56px] w-full max-w-3xl items-center gap-3 px-4">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-base font-semibold text-ink">{title ?? session.familyName}</h1>
          {title ? (
            <p className="truncate text-xs text-ink-faint md:hidden">{session.familyName}</p>
          ) : null}
        </div>
        {action}
