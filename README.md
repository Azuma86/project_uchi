...
> macOS なら `brew install openjdk@21`、その後
> `export JAVA_HOME=/opt/homebrew/opt/openjdk@21` を `.zshrc` などに追加してください
...
# 2. 環境変数を用意する (エミュレータ向けの値がそのまま使えます)
cp .env.example .env.local

# 3. Firebase Emulator を起動する (別ターミナルで起動したままにする)
npm run emulators
...
- `.env.local` の `USE_FIREBASE_EMULATORS=true` により、
  Admin SDK も クライアント SDK も **エミュレータにしか接続しません**
- `scripts/seed.ts` は冒頭で `FIRESTORE_EMULATOR_HOST` を確認し、
  ローカルを指していなければ **1 件も書き込まずに終了します**
- エミュレータのデータは `.emulator-data/` に保存され、`.gitignore` 済みです
